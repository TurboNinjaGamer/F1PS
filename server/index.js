...
/**
 * GET /api/track-svg?circuit=Suzuka
 *
 * Returns the pre-generated track SVG for a circuit.
 * Response: { ok: true, circuit: "Suzuka", svg: "<svg>...</svg>" }
 *
 * === Usage example (React) ===
 *
 *   // 1. Fetch:
 *   const res = await fetch("/api/track-svg?circuit=Suzuka");
 *   const { svg } = await res.json();
 *
 *   // 2. Parse viewBox and sector paths:
 *   const parser = new DOMParser();
 *   const doc = parser.parseFromString(svg, "image/svg+xml");
 *   const viewBox = doc.querySelector("svg").getAttribute("viewBox");
 *
 *   // 3. Render the track (simplest approach):
 *   <div dangerouslySetInnerHTML={{ __html: svg }} />
 *
 *   // 4. Place driver dots on the track (x, y from /api/realtime/track-map):
 *   //    The SVG coordinate system matches OpenF1 x,y — just negate y:
 *   <g transform={`translate(${driver.x}, ${-driver.y})`}>
 *     <circle r="220" fill={`#${driver.team_colour}`} />
 *     <text textAnchor="middle" y="55" fontSize="180" fill="#fff">
 *       {driverCode}
 *     </text>
 *   </g>
 *
 *   // 5. Full component example:
 *   function TrackMap({ trackSvg, driverPositions, driversMap }) {
 *     if (!trackSvg) return <div>No track map</div>;
 *     const parser = new DOMParser();
 *     const doc = parser.parseFromString(trackSvg, "image/svg+xml");
 *     const svgEl = doc.querySelector("svg");
 *     const viewBox = svgEl.getAttribute("viewBox");
 *     const paths = [...svgEl.querySelectorAll("path")].map(p => ({
 *       id: p.id, d: p.getAttribute("d"),
 *       stroke: p.getAttribute("stroke"),
 *     }));
 *     return (
 *       <svg viewBox={viewBox} style={{ width: "100%", maxWidth: 800 }}>
 *         {paths.map(p => (
 *           <path key={p.id} d={p.d} fill="none" stroke={p.stroke}
 *             strokeWidth="350" strokeLinecap="round" strokeLinejoin="round"
 *             opacity="0.85" />
 *         ))}
 ...
 *             <circle r="220"
 *               fill={`#${driversMap[d.driver_number]?.team_colour || "888"}`} />
 *           </g>
 *         ))}
 *       </svg>
 *     );
 *   }
 */
router.get("/track-svg", async (req, res) => {
  try {
    const circuit = req.query.circuit;
    if (!circuit) {
      return res.status(400).json({
        ok: false,
        error: "circuit query parameter required (e.g. ?circuit=Suzuka)",
      });
    }

    const [rows] = await db.execute(
      "SELECT circuit_short_name, track_svg FROM circuits WHERE circuit_short_name = ? LIMIT 1",
      [circuit]
    );

    if (rows.length === 0) {
      return res.status(404).json({ ok: false, error: `Circuit '${circuit}' not found` });
    }

    if (!rows[0].track_svg) {
      return res.status(404).json({ ok: false, error: `No SVG available for '${circuit}'` });
    }

    res.json({ ok: true, circuit: rows[0].circuit_short_name, svg: rows[0].track_svg });
  } catch (err) {
